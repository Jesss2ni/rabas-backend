...
router.get('/superAdmin-fetchAllUsers', async (req, res) => {
    ...
    try {
      ...
      const formattedUsers = results.map(user => ({
        ...
        name: `${user.Fname || ''} ${user.Lname || ''}`.trim(),  // Handle potential null values
        email: user.email || 'N/A',  // Default if email is null
      ...
      console.error('Error executing SQL query:', err.message || err);
